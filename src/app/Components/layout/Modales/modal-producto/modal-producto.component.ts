...
export class ModalProductoComponent implements OnInit{
  ...
  constructor(
    private _modalActual:MatDialogRef<ModalProductoComponent>,
    ...
    if(this.datosProducto == null){
      this._productoServicio.guardar(_producto).subscribe({
        next:(data)=>{
          ...
          }else this._utilidadServicio.mostrarAlerta("No se pudo registrar el producto","Error")
        },
        error:(e) => {}
      })
    }else{
      this._productoServicio.editar(_producto).subscribe({
        next:(data)=>{
          if (data.status){
            ...
            this._modalActual.close("true");
          }else this._utilidadServicio.mostrarAlerta("El producto no se pudo editar","Error")
        },
        error:(e) => {}
      })
    }
